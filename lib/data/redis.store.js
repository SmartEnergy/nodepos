...
function persistModel(key, model) {
  ...
  if(model instanceof Command) {
    ...
    app.logger.warn('No persist configuration found for '+ typeof(model));
  }
...
function doPersist (key, model, hashkey) {
  ...
  app.logger.info('Persisting '+hashkey+' Model with key: '+key);

  var value = JSON.stringify(model);

  redisClient.hset(hashkey, key, value, function(err, reply) {
    if(err) app.logger.error('Could not persist '+hashkey+' Model with key'+key);
  }); 

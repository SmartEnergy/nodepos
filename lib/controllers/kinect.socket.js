...
KinectSocket.prototype.entryPoint = function(socket) {
  ...
  KinectSocket.prototype.app.kinects.on('update', function(id, kinect) {
  ...
  KinectSocket.prototype.app.kinects.on('removed', function(id) {
    socket.write(JSON.stringify({
      cmd: "delete",
      object: "kinect",
      data: [id]
    ...
    case "new":
      ...
      if(users && users.length > 0) {
        ...
        for (var i = 0; i < users.length; i++) {
          var user = users[i];
          if(self.app.users.isValid(user) === false) {
            isValid = false;
          }
        };
        ...
        if(isValid === true) {
          ...
          this.response(socket, INVALIDREQ, "invalid users");
        }
      } else {
